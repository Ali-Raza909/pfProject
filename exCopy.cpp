...
slopeRightMirrorSprite.setTexture(slopeRightMirrorTexture);
        ...
        else if (currentLevel == 3)
        {
            ...
            // Phase 2: Pot on Cloud spawns enemies first, then Boss appears
            // ============================================================================
            ...
            // We want floor at around y = 850, so row = 850/42 ≈ 20
        ...
        while (window.isOpen() && !restartGame)
        {
            ...
            if (currentLevel == 3)
            {
                ...
                {
                    // --- UPDATE CLOUD MOVEMENT (moves up and down) ---
                    ...
                    // Update pot position to stay on cloud
                    potX = cloudX + cloudWidth / 2 - potWidth / 2;
                    potY = cloudY - potHeight;
                    
                    // --- SPAWN ENEMIES FROM POT (every 4 seconds) ---
                ...
                if (potDestroyed && !bossAppeared)
                {
                    ...
                    if (potDestroyTimer >= 1.5f)
                    {
                        ...
                        cout << "BOSS HAS APPEARED! The Octopus emerges!" << endl;
                    }
                }
                ...
                {
                    if (potEnemyIsCaught[pe]) continue;
                    
                    // Get enemy dimensions based on type
                    int enemyH = 60, enemyW = 72;  // Default ghost
                    if (potEnemyType[pe] == 2) { enemyH = 92; enemyW = 72; }   // Skeleton
                    else if (potEnemyType[pe] == 3) { enemyH = 80; enemyW = 60; }  // Invisible
                    else if (potEnemyType[pe] == 4) { enemyH = 90; enemyW = 60; }  // Chelnov
                ...
                {
                    if (!potEnemyProjActive[pp]) continue;
                ...
                if (bossAppeared && !bossDefeated)
                {
                    ...
                    if (minionSpawnTimer >= minionSpawnInterval && minionCount < maxMinions)
                    {
                        ...
                        for (int m = 0; m < minionsToSpawn && minionCount < maxMinions; m++)
                        {
                            ...
                            cout << "Minion spawned from boss! Total: " << minionCount << endl;
                        }
                    }
            ...
            if (currentLevel == 3 && potEnemyCount > 0 && isVacuuming)
            {
                ...
                for (int pe = 0; pe < potEnemyCount; pe++)
                {
                    ...
                    if (potEnemyType[pe] == 2) { enemyW = 72; enemyH = 92; }
                    else if (potEnemyType[pe] == 3) { enemyW = 60; enemyH = 80; }
                    else if (potEnemyType[pe] == 4) { enemyW = 60; enemyH = 90; }
                        ...
                        potEnemiesY[pe] + enemyH > vacTop && potEnemiesY[pe] < vacBottom)
                    {
                        ...
                        if (potEnemiesX[pe] < pCenterX) potEnemiesX[pe] += pullSpeed;
                        else potEnemiesX[pe] -= pullSpeed;
                        if (potEnemiesY[pe] < pCenterY) potEnemiesY[pe] += pullSpeed;
                        else potEnemiesY[pe] -= pullSpeed;
                        ...
                        if (dist < 50.0f * vacuumPower && capturedCount < MAX_CAPACITY)
                        {
                            // Capture the pot enemy
                            capturedEnemies[capturedCount] = potEnemyType[pe];  // Store enemy type
                            capturedCount++;
                            
                            // Award points based on type
                            int capturePoints = 50;
                            if (potEnemyType[pe] == 2) capturePoints = 75;
                            else if (potEnemyType[pe] == 3) capturePoints = 150;
                            else if (potEnemyType[pe] == 4) capturePoints = 200;
                            
                            addScore(playerScore, comboStreak, comboTimer, capturePoints, false, multiKillCount, multiKillTimer, dt);
                            
                            // Remove pot enemy by shifting array
                            ...
                            cout << "Captured pot enemy! Type: " << potEnemyType[pe+1] << " Total captured: " << capturedCount << endl;
                        }
                    }
                }
            }
            ...
            if (currentLevel == 3)
            {
                ...
                if (!projectileRemoved)
                {
                    for (int pe = 0; pe < potEnemyCount && !projectileRemoved; pe++)
                    {
                        ...
                        if (potEnemyType[pe] == 2) { enemyW = 72; enemyH = 92; }  // Skeleton
                        else if (potEnemyType[pe] == 3) { enemyW = 60; enemyH = 80; }  // Invisible
                        else if (potEnemyType[pe] == 4) { enemyW = 60; enemyH = 90; }  // Chelnov
                            ...
                            (projectilesY[p] + ProjectileHeight > potEnemiesY[pe]))
                        {
                            ...
                            if (potEnemyType[pe] == 2) defeatPoints = 75;   // Skeleton
                            else if (potEnemyType[pe] == 3) defeatPoints = 150;  // Invisible
                            else if (potEnemyType[pe] == 4) defeatPoints = 200;  // Chelnov
...
float Xoffset = (64 * scale - PlayerWidth) / 2.0f;
    ...
    if (potEnemiesX != NULL) { delete[] potEnemiesX; potEnemiesX = NULL; }
    if (potEnemiesY != NULL) { delete[] potEnemiesY; potEnemiesY = NULL; }
    if (potEnemySpeed != NULL) { delete[] potEnemySpeed; potEnemySpeed = NULL; }
    if (potEnemyDirection != NULL) { delete[] potEnemyDirection; potEnemyDirection = NULL; }
    if (potEnemyVelocityY != NULL) { delete[] potEnemyVelocityY; potEnemyVelocityY = NULL; }
    if (potEnemyOnGround != NULL) { delete[] potEnemyOnGround; potEnemyOnGround = NULL; }
    if (potEnemyIsCaught != NULL) { delete[] potEnemyIsCaught; potEnemyIsCaught = NULL; }
    if (potEnemyType != NULL) { delete[] potEnemyType; potEnemyType = NULL; }
    // Cleanup pot enemy behavior arrays
    if (potEnemyJumpTimer != NULL) { delete[] potEnemyJumpTimer; potEnemyJumpTimer = NULL; }
    if (potEnemyShouldJump != NULL) { delete[] potEnemyShouldJump; potEnemyShouldJump = NULL; }
    if (potEnemyStableFrames != NULL) { delete[] potEnemyStableFrames; potEnemyStableFrames = NULL; }
    if (potEnemyIsVisible != NULL) { delete[] potEnemyIsVisible; potEnemyIsVisible = NULL; }
    if (potEnemyVisibilityTimer != NULL) { delete[] potEnemyVisibilityTimer; potEnemyVisibilityTimer = NULL; }
    if (potEnemyTeleportTimer != NULL) { delete[] potEnemyTeleportTimer; potEnemyTeleportTimer = NULL; }
    if (potEnemyShootTimer != NULL) { delete[] potEnemyShootTimer; potEnemyShootTimer = NULL; }
    if (potEnemyIsShooting != NULL) { delete[] potEnemyIsShooting; potEnemyIsShooting = NULL; }
    
    // Cleanup dynamic captured enemies array (Phase 2)
    if (dynamicCapturedEnemies != NULL) { delete[] dynamicCapturedEnemies; dynamicCapturedEnemies = NULL; }

...
void generateLevel2Design(char **lvl, int platHeight, int platWidth)
{
    ...
    for (int p = 0; p < numPlatformRows; p++)
    {
        ...
        {
            if (checkRow < 0 || checkRow >= 12)
                continue;
            for (int j = 1; j <= 8; j++)
            {
                if (slantOccupied[checkRow][j])
                    leftClear = false;
            }
        }
...
int main()
{
    ...
    while (playagain)
    {
        ...
        // --- CHELNOV FIREBALL ANIMATION ---
        Texture chelnovProjTexture[4];
        chelnovProjTexture[0].loadFromFile("Data/chelnov/fireball1.png");
        chelnovProjTexture[1].loadFromFile("Data/chelnov/fireball2.png");
        chelnovProjTexture[2].loadFromFile("Data/chelnov/fireball3.png");
        chelnovProjTexture[3].loadFromFile("Data/chelnov/fireball4.png");

        Sprite chelnovProjSprite;
        chelnovProjSprite.setTexture(chelnovProjTexture[0]); // Set initial texture
        chelnovProjSprite.setScale(2.5f, 2.5f);
        
        // Animation state for each fireball projectile
        int chelnovProjAnimFrame[maxChelnovProjectiles];
        int chelnovProjAnimCounter[maxChelnovProjectiles];
        int chelnovProjAnimSpeed = 6; // Change frame every 6 game frames
            ...
            if (currentLevel == 2)
            {
                ...
                for (int i = 0; i < chelnovCount; i++)
                {
                    ...
                    if (chelnovShootTimer[i] >= 4.0f)
                    {
                        ...
                        if (chelnovProjCount < maxChelnovProjectiles)
                        {
                            ...
                            // Initialize animation for this fireball
                ...
                for (int i = 0; i < chelnovProjCount; i++)
                {
                    ...
                    if (chelnovProjX[i] < 0 || chelnovProjX[i] > screen_x)
                    {
                        ...
                        // Also copy animation state
                        chelnovProjAnimFrame[i] = chelnovProjAnimFrame[chelnovProjCount - 1];
                        chelnovProjAnimCounter[i] = chelnovProjAnimCounter[chelnovProjCount - 1];
                        chelnovProjCount--;
                        i--;
                        continue;
                    }
            ...
            if (currentLevel == 2)
            {
                ...
                for (int i = 0; i < chelnovProjCount; i++)
                {
                    if (chelnovProjActive[i])
                    {
                        ...
                        if (chelnovProjAnimCounter[i] >= chelnovProjAnimSpeed)
                        {
                            chelnovProjAnimCounter[i] = 0;
                        ...
                        chelnovProjSprite.setTexture(chelnovProjTexture[chelnovProjAnimFrame[i]], true);
                        
                        // Flip sprite based on direction
                        int texW = chelnovProjTexture[chelnovProjAnimFrame[i]].getSize().x;
                        int texH = chelnovProjTexture[chelnovProjAnimFrame[i]].getSize().y;
                        
                        if (chelnovProjDirection[i] == 1) // Moving right
                            chelnovProjSprite.setTextureRect(IntRect(0, 0, texW, texH));
                        else // Moving left - flip horizontally
                            chelnovProjSprite.setTextureRect(IntRect(texW, 0, -texW, texH));
